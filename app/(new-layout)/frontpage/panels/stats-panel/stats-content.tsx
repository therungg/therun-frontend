...
export function StatsContent({ initialStats, username, firstWeek, firstMonth }: Props) {
    ...
    const handleRangeChange = (newRange: 'week' | 'month', newOffset: number = 0) => {
        ...
        startTransition(async () => {
            const newStats = await fetchStatsAction(username, newRange, newOffset);
    ...
    return (
        <>
            <Row className="row-gap-3">
                <Col xxl={6} xl={12} className="d-flex justify-content-center align-items-center order-xxl-1 order-2">
                    <div style={{ opacity: isPending ? 0.5 : 1, transition: 'opacity 0.2s' }}>
                        <ProgressChart stats={stats} />
                    </div>
                </Col>

                <Col xxl={6} xl={12} className="order-xxl-2 order-1 d-flex flex-column">
                    <div className='flex-grow-1 d-flex justify-content-center align-items-center'>
                        <div style={{ opacity: isPending ? 0.5 : 1, transition: 'opacity 0.2s' }}>
                            <StatsHeader stats={stats} firstWeek={firstWeek} firstMonth={firstMonth} onRangeChange={handleRangeChange} />
                        </div>
                    </div>
