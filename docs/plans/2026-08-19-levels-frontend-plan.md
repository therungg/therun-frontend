...
- **Empty categories are shown (decision 2026-08-19).** `category_stats` (the source of `/v1/runs/categories`) only contains categories with runs. `resolveCategory` must union pageData's active categories with the stats rows: every pageData category without a stats row becomes a `ResolvedCategory` with zero stats (`totalRunTime: 0`, `totalFinishedAttemptCount: 0`, `uniqueRunners: 0`, `totalPbs: 0`, other fields from the pageData entry). The `isLowActivityCategory` filter stays only for rows that *have* stats (it hides auto-created timer noise); zero-stats categories are never dropped — not in `games-v1.ts`, not in `manage/page.tsx`.
...
- In `resolveCategory`: build `groups` with `kind: g.kind === 'level' ? 'level' : 'normal', rules: g.rules ?? null`; keep the full pageData entry per category id (`entryById`), not just flags; after mapping stat rows (still filtered by `isLowActivityCategory`), **append** a `ResolvedCategory` for every pageData category (ungrouped or grouped, any kind) that has no stats row: zero stats, `name: normalizeSlug(entry.display)`, `display`, `primaryTiming`/`gameTimeLabel`/`rules`/`showMilliseconds`/`requireVideo`/`sortAscending` from the entry (pageData entries carry these — see backend `rebuildGamePageData`), flags as for stats rows; set `levelTemplateId`/`levelOverride` on all rows from `entryById`; return `levelTemplates` mapped from `pageDataResp.result?.levelTemplates ?? []`. Add a test: a pageData category with no stats row appears with zero stats; a stats row below the activity floor is still dropped.
...
**Files:**
- Modify: `manage/console/board-categories-table.tsx` (~121 rows filter, ~143 bands), `manage/category/[categoryId]/page.tsx` + `category-detail.tsx`, `manage/page.tsx` (~122 low-activity drop — keep zero-stats rows), `manage/console/categories-pane.tsx` (add dialog pool)
...
- [ ] **Step 2: Run to verify fail.**
- [ ] **Step 3: Implement** — `splitLevelBoards(rows, groups)`; render level boards as one band with a disclosure (default collapsed; remember in `localStorage` key `console:levelBoards:${gameId}`); inside, sub-bands per level; reorder scope excludes level boards (their order follows the template). `manage/page.tsx`: keep zero-stats rows (only drop rows that have stats and fall under the activity floor); load `listLevelTemplates` and thread to the index and the detail page. Add-category dialog pool excludes level boards (they are featured via the template). Detail page: look up `levelTemplates`/`levelTemplateId`, render the banner + control; for a template hide the "Group" and "Featured" controls that don't apply (group is fixed null; Featured is pushed) — or leave Featured and explain it propagates.
- [ ] **Step 4: Run + tsc.**
...
- Decision (Joey, 2026-08-19): empty categories are shown everywhere; only stats-backed low-activity rows are still filtered.
