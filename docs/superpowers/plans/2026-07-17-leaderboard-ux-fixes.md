...
**Requirements:**
1. New mixins in `_board.scss`: `board-empty` (icon + title + hint, from the console `.empty`), `board-stepper` (from setup's stepper), `board-error-alert` (from the triplicated errorAlert), and `board-dialog-chrome` (header/title/body/textarea/error/footer — now triplicated across `run-action-dialog.module.scss`, `row-action-dialogs.module.scss`, `claim-cta.module.scss`, plus `self-run-verdict.module.scss`; reviewer-mandated, collapse all four onto the mixin). Replace the hand-copied clones: console `.pill` block (≈lines 347-377) → `@include board-pill`; needs-attention pill/amber recipes; reassignments' board-surface/eyebrow/pill/stepper clones; the three errorAlert copies; both empty-state variants converge on `board-empty`.
