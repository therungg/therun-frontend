...
export interface SetupStepMeta {
    id: SetupStepId;
    /** 1-10, matches the ghost numeral in the wizard's step header. */
    num: number;
    label: string;
    skippable: boolean;
    /**
...
export const SETUP_STEPS: SetupStepMeta[] = [
    ...
    { id: 'categories', num: 4, label: 'Categories', skippable: true },
    { id: 'levels', num: 5, label: 'Levels', skippable: true },
    { id: 'groups', num: 6, label: 'Category groups', skippable: true },
    {
        id: 'category-setup',
        num: 7,
    ...
    { id: 'verification', num: 9, label: 'Verification', skippable: false },
    {
        id: 'match-runners',
        num: 10,
        label: 'Match runners',
    ...
    { id: 'boards', num: 11, label: 'Boards', skippable: false, wide: true },
];
 ...
 *
 * `exceptions` folded into the per-category settings step; `defaults` became
 * the board-defaults half of step 1; `finish` is now the go-live footer under
 * board curation. (`variables` is a live step id again — the Subcategories &
 * filters step — so it resolves directly and is not in this map.)
 */
